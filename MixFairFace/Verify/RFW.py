...
def calculate_val(thresholds,
    ...
    for fold_idx, (train_set, test_set) in enumerate(k_fold.split(indices)):
        ...
        if np.max(far_train) >= far_target:
            f = interpolate.interp1d(far_train.copy(), thresholds, kind='slinear')
...
def load_dataset(lfw_dir, lfw_pairs_path, image_size):
    lfw_pairs = read_pairs(lfw_pairs_path)
    lfw_paths, issame_list = get_paths(lfw_dir, lfw_pairs, 'jpg')
    print (len(lfw_paths), len(issame_list))
    lfw_data_list = []
    for flip in [0, 1]:
        lfw_data = np.zeros((len(lfw_paths), 3, image_size[0], image_size[1]), np.float32)
    ...
    for path in lfw_paths:
        ...
        with open(path, 'rb') as fin:
            ...
            if i % 1000 == 0:
                print('loading lfw', i)
        '''
        img = (np.asarray(imread(path, pilmode='RGB'), np.float32) / 255).transpose(2, 0, 1)
        for flip in [0, 1]:
            if flip == 1: img = np.flip(img, axis=2)
            lfw_data_list[flip][i][:] = img
        i += 1
        if i % 1000 == 0: print('loading lfw', i)
...
def test(lfw_set, model, batch_size, config):
    print('testing lfw..')
    ...
    for i in range(len(lfw_data_list)):
        ...
        while ba < lfw_data.shape[0]:
            ...
            data = torch.FloatTensor(lfw_data[ba:bb, ...]).to(config['exp_args']['device'])
            with torch.no_grad(): _embeddings, _ = model(data)
            #print (_embeddings[0, ...].cpu().numpy())
            #exit()
            '''
            ...
            embeddings[ba:bb, :] = _embeddings.cpu().numpy()
            ba = bb
